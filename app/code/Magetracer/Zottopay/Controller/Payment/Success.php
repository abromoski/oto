...
class Success extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
		...
		{
			$order = $this->_orderFactory->create()->loadByIncrementId($_REQUEST['key']);
			$model = $this->_paymentMethod;      
			
			$order->setState($model->getConfigData('success_order_status'));
			$order->setStatus($model->getConfigData('success_order_status'));
			$order->addStatusToHistory($model->getConfigData('success_order_status'), __(self::BrowserReturn.'Payment Success!'));
