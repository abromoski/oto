...
class Index extends Action
{
	...
	private function getCustomerInfo($id,$LUD){
		...
		if($cust->getData('default_billing') != 0)
		{
			...
			$billingAddress['BillingAddress'] = array(
					...
					'Line3'=>htmlspecialchars(isset($streetBill[2])?$streetBill[2]:""),
		...
		if($cust->getData('default_shipping') != 0)
		{
			...
			$shippingAddress['ShippingAddress'] = array(
					...
					'OrganizationName'=>htmlspecialchars($custShippingAdd->getCompany()),
					'WorkPhone'=>htmlspecialchars($custShippingAdd->getTelephone()),									
					'Line1'=>htmlspecialchars(isset($streetShip[0])?$streetShip[0]:""),
					'Line2'=>htmlspecialchars(isset($streetShip[1])?$streetShip[1]:""),
					'Line3'=>htmlspecialchars(isset($streetShip[2])?$streetShip[2]:""),
			...
			foreach($items as $item){
				...
				if($orderItemId == $parentItemId)
				{
					$item_arr_child = array(
						...
						'Price'=>htmlspecialchars($item->getBasePriceInclTax()),
						'UnitPriceExTax'=>htmlspecialchars($item->getBasePrice()));
						if ($item->getBaseDiscountAmount()>0) {$item_arr_child['Discount'] = htmlspecialchars($item->getBaseDiscountAmount()/$item->getQtyOrdered());}
						if ($item->getBaseTaxAmount()>0) {
							$tax = $this->getTaxCodeForItem($order,$item->getTaxPercent());
	...
	private function getOrderInfo($id,$original_id, $LUD){
		...
		$shippings_array = array(
			...
			'Amount'=>htmlspecialchars($order->getBaseShippingInclTax()));
			if($order->getBaseShippingTaxAmount() > 0)
				{
					...
					$shippings_array['Taxes']= array(
						...
						'TaxCode'=>$tax['code'],
						'TaxName'=>$tax['title'],
						'TaxRate'=>htmlspecialchars((0.0 + $tax['tax_percent'])/100),
						'TaxAmount'=>htmlspecialchars($order->getBaseShippingTaxAmount()));				
				}
			else
				{
		...
		foreach($credits as $key => $a_credit) {
			$credits_array[] = array('Date'=>htmlspecialchars($a_credit->getCreatedAt()),'DateUpdated'=>htmlspecialchars($a_credit->getUpdatedAt()), 'Amount'=>htmlspecialchars($a_credit->getBaseGrandTotal()));
		}	
		...
		if ($custShippingAdd && $custBillingAdd) {
			...
			$contact_info = array(
							'Id' => htmlspecialchars(is_null($order->getCustomerId())?"0":$order->getCustomerId()),
							'Salutation' => htmlspecialchars($salutation),
		...
		$ord_info = array(
							...
							'LastUpdatedDate' => htmlspecialchars($order->getUpdatedAt()),
							'Type'=>"Order",
	...
	private function getConfigCollection(){
		...
		foreach($tr_data as $tr){
			...
			$trs['TaxCodes'][]=array('Name'=>htmlspecialchars($tr['code']), 'Rate'=>(0.0 + $tr['rate'])/100, 'CountryCode'=>htmlspecialchars($tr['tax_country_id']), 'Zip'=>htmlspecialchars($tr['tax_postcode']));
		}
        ...
        foreach ($carriers as $carrierCode => $carrierModel) {	
			...
			$ShippingMethods['ShippingMethods'][]=array('Name'=>htmlspecialchars($carrierCode), 'Description'=>htmlspecialchars(strip_tags($carrierTitle)));
        }
		...
		foreach ($orderstatuses as $orderstatus){	
			$OrderStatuses['OrderStatuses'][]=array('Name'=>$orderstatus['status'], 'Description'=>htmlspecialchars($orderstatus['label']));
		}
		...
		if($price_setting === '0')
			{
				$storeInfo_array['IsSellingPriceIncludingTax'] = 'false';
			}
		else
			{
				$storeInfo_array['IsSellingPriceIncludingTax'] = 'true';
			}
