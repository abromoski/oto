 ...
 * @copyright  Copyright (c) 2017 Paytriot
 * @license    http://opensource.org/licenses/osl-3.0.php  Open Software License (OSL 3.0)
 */
namespace Paytriot\PaymentGateway\Model;

class Gateway extends \Magento\Payment\Model\Method\AbstractMethod {
	const _MODULE = 'Paytriot_PaymentGateway';
	const DIRECT_URL = 'https://gateway.paytriot.co.uk/direct/';
	const HOSTED_URL = 'https://gateway.paytriot.co.uk/paymentform/';
			...
			if ($data['responseCode'] == 65802) {
				...
				if ($data['responseCode'] == 0){
					...
					if($order->getStatus() != $status) {
						...
						$objectManager->create('Paytriot\PaymentGateway\Model\Trans')
	...
	private function buildStatus($data) {
		$ordermessage = "Paytriot Payment<br/><br/>" .
	...
	public function getOrderPlaceRedirectUrl() {
		return $this->_urlBuilder->getUrl('paytriot/order/process');
	}
}
