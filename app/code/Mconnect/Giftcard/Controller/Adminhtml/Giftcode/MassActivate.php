...
class MassActivate extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        foreach ($collection as $giftcode) {
            ...
            if ($status == \Mconnect\Giftcard\Model\Giftcode::STATUS_INACTIVE) {
                $giftcode->setData('status', \Mconnect\Giftcard\Model\Giftcode::STATUS_ACTIVE);
                $giftcode->save();
                $count++;
            }
        }

        $this->messageManager->addSuccess(__('A total of %1 record(s) have been activated.', $count));
