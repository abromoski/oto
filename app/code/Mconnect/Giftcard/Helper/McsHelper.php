...
class McsHelper extends \Magento\Framework\App\Helper\AbstractHelper
{
	...
	public function checkLicenceKeyActivation($storeId = null)
    {
		...
		if($this->getLicenceKey() && $this->getSerialKey()){
			...
			if( !(($this->simple_encrypt($this->getRequestHost()."Mconnect_Giftcard",substr($this->getLicenceKey(),1,16)) == $this->getSerialKey()) || ($this->simple_encrypt($this->getWebsiteHost()."Mconnect_Giftcard",substr($this->getLicenceKey(),1,16)) == $this->getSerialKey())) ){
	...
	public function getLicenceKey(){
		return $this->scopeConfigObject->getValue("giftcard/active/licence_key", \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $this->getStore());
	}	
	public function getSerialKey(){
		return $this->scopeConfigObject->getValue("giftcard/active/serial_key", \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $this->getStore());
	}
